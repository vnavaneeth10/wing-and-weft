...
const STYLES = `
  ...
  @keyframes nf-ring-pulse2 {
    0%,100%{transform:translate(-50%,-50%) scale(1);opacity:0.07}
    50%{transform:translate(-50%,-50%) scale(1.12);opacity:0.16}
  }
  ...
  @keyframes nf-cursor-ring {
    from{transform:translate(-50%,-50%) scale(0.3);opacity:0}
    to{transform:translate(-50%,-50%) scale(1);opacity:0}
  }

  .nf-shimmer {
    background:linear-gradient(90deg,#bc3d3e 0%,#e69358 20%,#f0c070 35%,#b6893c 50%,#f0c070 65%,#e69358 80%,#bc3d3e 100%);
  ...
  .nf-cta-primary:hover{transform:translateY(-4px);letter-spacing:0.25em;box-shadow:0 16px 48px rgba(188,61,62,0.55),0 4px 12px rgba(0,0,0,0.3);}
  ...
  .nf-cta-ghost::after {
    ...
    background:rgba(182,137,60,0.08);
  ...
  return (
    <>
      <div ref={dotRef} style={{
        ...
        background:'#bc3d3e',pointerEvents:'none',zIndex:9999,
        boxShadow:'0 0 8px rgba(188,61,62,0.8)',
      }}/>
      <div ref={ringRef} style={{
        position:'fixed',top:0,left:0,width:'32px',height:'32px',borderRadius:'50%',
        border:`1px solid ${dark ? 'rgba(182,137,60,0.6)' : 'rgba(188,61,62,0.5)'}`,
...
// ─── Animated thread SVG ──────────────────────────────────────────────────────
const ThreadCanvas: React.FC<{ dark: boolean }> = ({ dark }) => (
  <svg viewBox="0 0 560 140" fill="none"
    ...
    <path d="M0,70 C70,20 100,110 180,60 C260,10 290,100 360,55 C430,10 470,90 560,65"
      stroke="url(#tg1)" strokeWidth="2" strokeLinecap="round" strokeDasharray="900"
    ...
    <path d="M0,85 C60,45 110,115 190,75 C270,35 300,105 375,70 C450,35 500,95 560,80"
      stroke="url(#tg2)" strokeWidth="1.2" strokeLinecap="round" strokeDasharray="900" opacity="0.6"
      style={{animation:'nf-thread2 3s ease 0.6s both'}}/>
    <path d="M0,55 C80,25 120,95 195,50 C270,5 310,85 385,45 C460,5 505,75 560,55"
      stroke="url(#tg3)" strokeWidth="0.8" strokeLinecap="round" strokeDasharray="900" opacity="0.35"
      style={{animation:'nf-thread3 3s ease 0.6s both'}}/>
    {/* Bobbin ends */}
    <circle cx="0"   cy="70" r="5" fill="#bc3d3e" opacity="0.7" style={{animation:'nf-bobbin 3s ease-in-out infinite'}}/>
    <circle cx="560" cy="65" r="5" fill="#b6893c" opacity="0.7" style={{animation:'nf-bobbin 3s ease-in-out 1.5s infinite'}}/>
    <defs>
      <linearGradient id="tg1" x1="0%" y1="0%" x2="100%" y2="0%">
        <stop offset="0%"   stopColor="#bc3d3e" stopOpacity="0"/>
        <stop offset="25%"  stopColor="#bc3d3e"/>
        <stop offset="75%"  stopColor="#b6893c"/>
        <stop offset="100%" stopColor="#b6893c" stopOpacity="0"/>
      </linearGradient>
      <linearGradient id="tg2" x1="0%" y1="0%" x2="100%" y2="0%">
        <stop offset="0%"   stopColor="#b6893c" stopOpacity="0"/>
        <stop offset="50%"  stopColor="#e69358"/>
        <stop offset="100%" stopColor="#bc3d3e" stopOpacity="0"/>
      </linearGradient>
      <linearGradient id="tg3" x1="0%" y1="0%" x2="100%" y2="0%">
        <stop offset="0%"   stopColor="#bc3d3e" stopOpacity="0"/>
        <stop offset="40%"  stopColor="#f0c070"/>
        <stop offset="100%" stopColor="#b6893c" stopOpacity="0"/>
      </linearGradient>
    </defs>
  </svg>
...
const NotFoundPage: React.FC = () => {
  ...
  const textMain  = dark ? '#f0e8d6'  : '#1a1410';
  const textSub   = dark ? 'rgba(240,232,214,0.45)' : 'rgba(26,20,16,0.38)';
  const cardBg    = dark ? 'rgba(18,12,8,0.75)'     : 'rgba(255,255,255,0.72)';
  const cardBdr   = dark ? 'rgba(182,137,60,0.18)'  : 'rgba(182,137,60,0.28)';
  ...
  return (
    <div onClick={burst} style={{
      ...
      />
        ...
        {[260,380,500].map((r,i) => (
          <div key={r} style={{
            ...
            width:r, height:r,
            border:`1px solid ${dark ? 'rgba(188,61,62,0.12)' : 'rgba(188,61,62,0.08)'}`,
            borderRadius:'50%',
        ...
        <div style={{
          ...
          background:dark
            ? 'radial-gradient(circle,rgba(188,61,62,0.18) 0%,rgba(182,137,60,0.06) 45%,transparent 70%)'
            : 'radial-gradient(circle,rgba(188,61,62,0.1) 0%,rgba(182,137,60,0.04) 45%,transparent 70%)',
        ...
        <div style={{
          ...
          borderRadius:'50%',
          background:dark
            ? 'radial-gradient(circle,rgba(182,137,60,0.14) 0%,transparent 65%)'
            : 'radial-gradient(circle,rgba(182,137,60,0.08) 0%,transparent 65%)',
        ...
        {[
          ...
          {w:130,h:170,x:'70%',y:'68%',anim:'nf-drift3 12s ease-in-out 4s infinite',op:dark?0.4:0.2},
          {w:180,h:140,x:'-2%',y:'68%',anim:'nf-drift1 16s ease-in-out 1s infinite',op:dark?0.5:0.25},
          {w:90, h:110,x:'50%',y:'3%', anim:'nf-drift2 20s ease-in-out 3s infinite',op:dark?0.3:0.15},
        ].map((s,i) => (
          <div key={i} style={{
            ...
            background:i%2===0
              ? `radial-gradient(ellipse at 35% 35%,${dark?'rgba(188,61,62,0.4)':'rgba(188,61,62,0.22)'} 0%,${dark?'rgba(182,137,60,0.15)':'rgba(182,137,60,0.1)'} 50%,transparent 75%)`
              : `radial-gradient(ellipse at 65% 35%,${dark?'rgba(182,137,60,0.35)':'rgba(182,137,60,0.2)'} 0%,${dark?'rgba(188,61,62,0.1)':'rgba(188,61,62,0.08)'} 50%,transparent 75%)`,
            opacity:s.op,
            animation:s.anim,
            filter:'blur(2px)',
          }}/>
        ...
        {/* Orbiting particles */}
        {[
          {r:240,size:7,dur:'16s',color:'#bc3d3e',cw:true,delay:0},
          {r:300,size:4,dur:'22s',color:'#b6893c',cw:false,delay:-5},
          {r:200,size:5,dur:'12s',color:'#e69358',cw:true,delay:-8},
          {r:340,size:3,dur:'28s',color:'#bc3d3e',cw:false,delay:-12},
        ].map((o,i) => (
          <div key={i} style={{
            position:'absolute',top:'42%',left:'50%',
      ...
      <div style={{
        ...
        boxShadow: dark
          ? '0 32px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(182,137,60,0.1), inset 0 1px 0 rgba(255,255,255,0.04)'
          : '0 24px 64px rgba(26,20,16,0.12), 0 0 0 1px rgba(182,137,60,0.12)',
        opacity:  mounted ? 1 : 0,
        ...
        <div style={{position:'absolute',top:0,left:0,width:'48px',height:'48px',pointerEvents:'none'}}>
          <div style={{position:'absolute',top:'16px',left:'16px',width:'24px',height:'1px',background:'rgba(188,61,62,0.5)'}}/>
          <div style={{position:'absolute',top:'16px',left:'16px',width:'1px',height:'24px',background:'rgba(188,61,62,0.5)'}}/>
        </div>
        {/* Bottom right corner accent */}
        <div style={{position:'absolute',bottom:0,right:0,width:'48px',height:'48px',pointerEvents:'none'}}>
          <div style={{position:'absolute',bottom:'16px',right:'16px',width:'24px',height:'1px',background:'rgba(182,137,60,0.5)'}}/>
          <div style={{position:'absolute',bottom:'16px',right:'16px',width:'1px',height:'24px',background:'rgba(182,137,60,0.5)'}}/>
        </div>
        ...
        <div style={{
          ...
          <div style={{width:'32px',height:'1px',background:'linear-gradient(to right,transparent,#bc3d3e)'}}/>
          <span style={{
            fontFamily:'"Raleway",sans-serif',fontSize:'0.58rem',fontWeight:700,
            letterSpacing:'0.42em',textTransform:'uppercase',color:'#bc3d3e',
          }}>Wing &amp; Weft</span>
          <div style={{width:'32px',height:'1px',background:'linear-gradient(to left,transparent,#bc3d3e)'}}/>
        </div>

        {/* 404 — perspective container */}
        <div style={{
          perspective:'800px',
          display:'flex',
          ...
          {/* 0 — shimmer */}
          <span className="nf-shimmer" style={{
            fontFamily:'"Cormorant Garamond",serif',
            fontSize:'clamp(7.5rem,19vw,12rem)',
            fontWeight:300, display:'inline-block',
            animation:'nf-zero-spin 1s cubic-bezier(0.22,1,0.36,1) 0.43s both',
          }}>0</span>
          {/* 4 */}
          <span style={{
            fontFamily:'"Cormorant Garamond",serif',
        ...
        <div style={{
          ...
          <div style={{width:'40px',height:'1px',background:'linear-gradient(to right,transparent,#b6893c)'}}/>
          <span style={{color:'#b6893c',fontSize:'0.45rem',opacity:0.8}}>◆</span>
          <div style={{width:'40px',height:'1px',background:'linear-gradient(to left,transparent,#b6893c)'}}/>
        </div>
        ...
        <div style={{
          ...
          <Link to="/" className="nf-cta-primary" style={{
            ...
            boxShadow:'0 6px 28px rgba(188,61,62,0.4),inset 0 1px 0 rgba(255,255,255,0.08)',
          }}>
            Return Home
          ...
          <Link to="/contact" className="nf-cta-ghost" style={{
            ...
            color: dark ? 'rgba(233,227,203,0.65)' : 'rgba(26,20,16,0.5)',
            fontFamily:'"Raleway",sans-serif',
